...
Baseline 122/122. Expected at the end ≈ 152. Frontend: `cd frontend && npm run generate:tokens && npm test` (48/48) and
...
- [ ] T016 Real-instance acceptance: rebuild or `zpm load`, run the full backend suite (record the count, ≈ 152, 0 failures), frontend unit 48/48 and e2e 13/13, then execute [quickstart.md](quickstart.md) (a)–(e) including the **mandatory (e)4**, saving status + body per call (no tokens, no passwords) to `specs/005-declared-custom-steps/evidence/quickstart-http-<date>.json`; confirm residue: no `SentaiTask:` native tasks left, temporary users deleted.
