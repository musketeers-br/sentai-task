...
Baseline 122/122 when these tasks were written; **178/178 now that spec 006 and its close-out have landed** (it added
`StepType.DestructiveByClass`, fixed `purge-audit-records`' class to `%SYS.Task.PurgeAudit`, and
added tests in `StepTypeTest` and `StepAvailabilityRuleTest`; the close-out made the token gate pass a platform 403 and moved `/schedule` to `POST /api/admin/v2/task` via `sentai.dispatch.NativeScheduler` — T010's `IN_PROCESS_NOT_SCHEDULABLE` goes in `ValidateForSchedule`, before that call). Expected at the end ≈ 208 (178 + the
...
**Cross-spec edges** (Constitution V):

```text
spec 003 token gate accepts the platform's 403 (done with spec 006 close-out) ─► T013 (quickstart (e)4)
T013 ─► spec 007 T009 e2e (gate G-C, temporary flip) ─► T009 merged ─► T016 commits purge-task-history available:true
```
